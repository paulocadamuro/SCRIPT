...
-- A table containing item IDs that should not be sold.
local DoNotSell = {
    --  IMBUIMENTS
    ...
    ------ EK --------
    44602, -- lesser guardian gem
    44603, -- guardian gem
    44604, -- greater guardian gem

    ------ RP ------
    44605, -- lesser marksman gem
    44606, -- marksman gem
    44607, -- greater marksman gem

    ------ ED ------
    44608, -- lesser sage gem
    44609, -- sage gem
    44610, -- greater sage gem

    ------ MS -------
    44611, -- lesser mystic gem
    44612, -- mystic gem
    44613, -- greater mystic gem

}

--- Checks if a specific item ID can be sold.
-- This function iterates over the DoNotSell table to determine if the provided item ID is in the list of items not to be sold.
...
--- Checks the amount of a specific item in the player's inventory.
-- @param itemID (number) - The ID of the item to check.
-- @return (number) The amount of the specified item in the player's inventory.
function CheckItem(itemID)
    local Amount = Game.getItemCount(itemID)
    return Amount
end

-- Other variables and flags for controlling the selling process.
local StartAutomatically = false
local IsSelling = false
local KeySwtich = "p"
local SellingDelay = 720

--- Sells all sellable items in the `itemList` array to an NPC.
-- This function checks each item in the `itemList`, verifies if it can be sold (is not in the `DoNotSell` list),
-- and if the player possesses it, then proceeds to sell all available quantities to the NPC.
-- A client message is displayed upon completion of the selling process.
function SellAllItens()
    ...
    gameTalk("trade", 12)  -- Proceed to the trade dialogue.
    IsSelling = true
    local CanSell = {}
    local PlayersInventory = GetPlayersInventory()

    -- Check each item in the `itemList`. If it can be sold and the player has it, add it to the `CanSell` list.
    for i=1,#itemList do
        local itemId = itemList[i]
        if CheckCanSellId(itemId) then  -- Verify if the item can be sold.
            local Amount = CheckItem(itemId)  -- Get the quantity of the item the player has.
            if Amount and Amount > 0 then
                if PlayersInventory[tostring(itemId)] then
                    Amount = Amount - 1
                end
                table.insert(CanSell, {itemId, Amount})  -- Add the item and its quantity to the `CanSell` list.
            end
        end
    end

    -- Sell each item in the `CanSell` list to the NPC and wait for `SellingDelay` milliseconds between each transaction.
    for i=1,#CanSell do
        local itemId = CanSell[i][1]
        local Amount = CanSell[i][2]
        -- if amount > 250 then cut the amount in small sells of 250
        if Amount > 100 then
    ...
    Client.showMessage("Finish Selling")  -- Display a completion message in the client.x
    IsSelling = false  -- Reset the selling flag.
    return true
end

-- A timer function that listens for the `KeySwtich` hotkey.
-- If pressed, it triggers the `SellAllItens` function to start the selling process, provided it is not already active.
-- This check occurs every 50 milliseconds.
Timer("keys", function()
    local status, modifiers, key = HotkeyManager.parseKeyCombination(KeySwtich)
    if status then
        if Client.isKeyPressed(key, modifiers) then
            if not IsSelling then  -- Ensure the selling process is not already ongoing.
                SellAllItens()  -- Start the selling process.
                Client.showMessage("Start selling all the itens")  -- Notify the player that the selling process has begun.
            end
        end
    end
end, 50)

-- A timer function named "SellAutomatically" that automatically triggers the `SellAllItens` function at a specified interval.
-- This timer only starts if the `StartAutomatically` flag is set to true.
-- @param function - The function to be called when the timer triggers, in this case, `SellAllItens`.
-- @param interval (number) - The time interval in milliseconds at which the timer triggers the function. Here, it's set to 100ms.
-- @param autoStart (boolean) - A flag that determines if the timer should start automatically. It's controlled by the `StartAutomatically` variable.
Timer("SellAutomatically", function()
    Client.showMessage("Start selling all the itens")  -- Notify the player that the selling process has begun.
    SellAllItens() -- Calls the function to sell all items in the inventory that are not in the `DoNotSell` list.
end, 100, StartAutomatically) -- The timer triggers every 100ms and only starts automatically if `StartAutomatically` is true.